...
void ParticleEmitter::Update()
{
	...
	for (int i = 0; i < PARTICLE_MAX; i++) {
		if (m_Particle[i].Enable == false){
			...
			m_Particle[i].Life = 250;
			m_Particle[i].Position = m_position;

			m_Particle[i].Velocity.x = (rand() % 500 - 50) / 500.0f;
			m_Particle[i].Velocity.y = (rand() % 100 + 50) / 500.0f;
			m_Particle[i].Velocity.z = (rand() % 500 - 50) / 500.0f;
...
void ParticleEmitter::Draw()
{
	...
	material.Diffuse = XMFLOAT4(0.0f, 0.1f, 1.0f, 1.0f);
